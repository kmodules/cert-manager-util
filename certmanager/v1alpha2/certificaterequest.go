...
func UpdateCertificateRequestStatus(
	...
	err = wait.PollImmediate(kutil.RetryInterval, kutil.RetryTimeout, func() (bool, error) {
		attempt++
		var e2 error
		result, e2 = c.CertificateRequests(in.Namespace).UpdateStatus(apply(cur))
		if kerr.IsConflict(e2) {
			latest, e3 := c.CertificateRequests(in.Namespace).Get(in.Name, metav1.GetOptions{})
			switch {
			case e3 == nil:
				cur = latest
				return false, nil
	...
	if err != nil {
		err = fmt.Errorf("failed to update status of CertificateRequest %s/%s after %d attempts due to %v", in.Namespace, in.Name, attempt, err)
	}
	return
}

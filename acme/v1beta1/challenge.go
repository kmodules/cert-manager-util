...
func CreateOrPatchChallenge(ctx context.Context, c cs.AcmeV1beta1Interface, meta metav1.ObjectMeta, transform func(alert *api.Challenge) *api.Challenge, opts metav1.PatchOptions) (*api.Challenge, kutil.VerbType, error) {
	cur, err := c.Challenges(meta.Namespace).Get(ctx, meta.Name, metav1.GetOptions{})
	if kerr.IsNotFound(err) {
		klog.V(3).Infof("Creating Challenge %s/%s.", meta.Namespace, meta.Name)
...
func PatchChallenge(ctx context.Context, c cs.AcmeV1beta1Interface, cur *api.Challenge, transform func(*api.Challenge) *api.Challenge, opts metav1.PatchOptions) (*api.Challenge, kutil.VerbType, error) {
	return PatchChallengeObject(ctx, c, cur, transform(cur.DeepCopy()), opts)
}

func PatchChallengeObject(ctx context.Context, c cs.AcmeV1beta1Interface, cur, mod *api.Challenge, opts metav1.PatchOptions) (*api.Challenge, kutil.VerbType, error) {
	...
	klog.V(3).Infof("Patching Challenge %s/%s with %s.", cur.Namespace, cur.Name, string(patch))
	out, err := c.Challenges(cur.Namespace).Patch(ctx, cur.Name, types.MergePatchType, patch, opts)
	return out, kutil.VerbPatched, err
}

func TryUpdateChallenge(ctx context.Context, c cs.AcmeV1beta1Interface, meta metav1.ObjectMeta, transform func(*api.Challenge) *api.Challenge, opts metav1.UpdateOptions) (result *api.Challenge, err error) {
	attempt := 0
	err = wait.PollImmediate(kutil.RetryInterval, kutil.RetryTimeout, func() (bool, error) {
		...
		klog.Errorf("Attempt %d failed to update Challenge %s/%s due to %v.", attempt, cur.Namespace, cur.Name, e2)
		return false, nil
	})
